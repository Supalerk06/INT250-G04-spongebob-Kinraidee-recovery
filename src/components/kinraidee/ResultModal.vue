...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white text-slate-900 p-2 rounded-[3.5rem] w-full max-w-lg shadow-[0_0_100px_rgba(0,0,0,0.2)] transform transition-all animate-popIn overflow-hidden">
      ...
      <div class="relative h-64 w-full overflow-hidden rounded-[2.8rem]">
        ...
        <div class="absolute top-6 right-6">
          <button @click="emit('close')" class="w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-slate-800 shadow-lg hover:bg-white transition-colors">
      ...
      <div class="p-10 flex flex-col items-start gap-4">
        <h2 class="text-4xl font-black text-slate-900 leading-tight">{{ result.name }}</h2>
