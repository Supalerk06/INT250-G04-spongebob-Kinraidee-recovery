...
const allRestaurants = computed(() => {
  return foods.flatMap(f => f.restaurants)
})

const food = computed(() => {
  return foods.find(f => f.name === targetMenu.value)
})
...
<template>
  ...
  <div class="absolute inset-0 bg-gradient-to-br from-red-900 via-red-950 to-black opacity-95 -z-10"></div>
  <div class="absolute top-[-10%] right-[-5%] w-[40vw] h-[40vw] bg-yellow-500/20 blur-[120px] rounded-full -z-10"></div>
  <div class="absolute bottom-[-20%] left-[-10%] w-[30vw] h-[30vw] bg-red-600/30 blur-[100px] rounded-full -z-10"></div>
  <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay -z-10">
  </div>

<header
  class="relative bg-gradient-to-r from-red-700 to-red-900 pt-5 pb-28  rounded-b-[70px] border-b border-red-500/40
  shadow-[0_10px_40px_rgba(0,0,0,0.6)]">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
          <div class="space-y-1">
            <div
              ...
              <span class="w-2 h-2 bg-primary rounded-full animate-pulse shadow-[0_0_8px_#FFEB3B]"></span>
              Current Location: KMUTT
            </div>
            <h1 class="text-4xl md:text-6xl font-display font-black tracking-tight text-white leading-none">
              ผัดกะเพรา
            </h1>
          </div>
          ...
          <!-- distance box  -->
          <div
            class="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto bg-white/10 backdrop-blur-xl p-5 rounded-3xl border border-white/20 shadow-xl text-white">
            <div class="w-full sm:w-60">
              <div class="flex justify-between items-center mb-3">
                <span class="text-[10px] font-bold text-red-100 uppercase tracking-widest font-display">
                  ระยะทางค้นหา
                </span>
            ...
            <div class="relative flex flex-col gap-1 w-full sm:w-[140px]">
              ...
              <select
                class="appearance-none bg-transparent font-bold text-white text-sm focus:outline-none cursor-pointer pb-1 border-b border-white/30 hover:border-primary transition-colors pr-6">
    ...
    <main class="relative z-20 max-w-7xl mx-auto px-6 -mt-20">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div v-for="(shop, index) in allRestaurants" :key="index"
          class="bg-white rounded-[2.5rem] p-3 shadow-xl hover:shadow-[0_10px_30px_rgba(255,215,0,0.25)]
 hover:-translate-y-2 transition-all duration-500 group border border-gray-100 flex flex-col">
          ...
          <div class="px-2 pb-2 flex-1 flex flex-col justify-between">
            <h3
              class="font-display font-black text-xl leading-tight text-text group-hover:text-secondary transition-colors line-clamp-1 mb-4">
              {{ shop.name }}
            </h3>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
              <div class="flex flex-col">
                <span
                  class="text-[9px] font-bold text-gray-400 uppercase tracking-widest font-display mb-0.5">Distance</span>
                <span class="text-xl font-display font-black text-secondary leading-none">
                  {{ shop.distance }} KM
                </span>
              </div>
              <button
