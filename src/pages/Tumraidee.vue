...
import RecipePopup from "@/components/tumraidee/RecipePopup.vue";

const route = useRoute();
const autoOpenRecipe = ref(null);

onMounted(() => {
  const recipeId = route.query.recipeId;
  if (recipeId) {
    const found = recipes.value.find((r) => r.id == recipeId);
    if (found) {
      autoOpenRecipe.value = found;
    }
  }
});

function handleAutoCook() {
  
  autoOpenRecipe.value = null;
}
...
<template>
  <div
    class="bg-slate-50 dark:bg-background-dark font-sans text-slate-900 dark:text-white min-h-screen flex flex-col overflow-x-hidden relative transition-colors duration-300">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <main class="lg:col-span-9 flex flex-col gap-8">
          ...
          <div v-if="availableRecipes.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10">
            <PerfectMatch v-for="Perfect in availableRecipes" :fridgeItems="fridgeItems" :id="Perfect.id"
              :name="Perfect.name" :short_description="Perfect.short_description" :difficulty="Perfect.difficulty"
              :image="Perfect.image" :video="Perfect.video" :ingredients="Perfect.ingredients" :steps="Perfect.steps"
              :categories="Perfect.categories" :categoryMapping="categoryMapping"></PerfectMatch>
          </div>
          <div v-else
            class="flex flex-col items-center justify-center p-8 bg-white dark:bg-card-dark rounded-3xl border border-dashed border-gray-200 dark:border-card-dark-02">
          ...
          <div v-if="almostReadyRecipes.length" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-5">
            <AlmostMatch v-for="Almost in almostReadyRecipes" :id="Almost.id" :name="Almost.name" :image="Almost.image"
              :missList="Almost.missingList" :missingCount="Almost.missingCount" :categories="Almost.categories"
          ...
          <div
            class="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-card-dark p-6 rounded-3xl border border-gray-100 dark:border-card-dark-02 shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 gap-4 transition-colors">
            <p class="flex gap-2 text-xl font-black text-slate-800 dark:text-white">
          ...
          <RecipePopup 
            v-if="autoOpenRecipe" 
            :name="autoOpenRecipe.name" 
            :image="autoOpenRecipe.image" 
            :video="autoOpenRecipe.video" 
            :ingredients="autoOpenRecipe.ingredients" 
            :steps="autoOpenRecipe.steps"
            @close="autoOpenRecipe = null" 
            @cook="handleAutoCook" 
          />
          ...
          <div v-if="userRecipes.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <MyRecipe v-for="(myRecipes, index) in userRecipes" :fridgeItems="fridgeItems" :id="myRecipes.id"
              :name="myRecipes.name" :short_description="myRecipes.short_description" :image="myRecipes.image"
              :ingredients="myRecipes.ingredients" :steps="myRecipes.steps" @delete="deleteRecipe" />
          </div>
          <div v-else
            class="flex flex-col items-center justify-center p-8 bg-white dark:bg-card-dark rounded-3xl border border-dashed border-gray-200 dark:border-card-dark-02">
